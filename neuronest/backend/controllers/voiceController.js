...
const processVoiceCommand = asyncHandler(async (req, res) => {
  ...
  // Use AI intent classification (falls back to regex if HF unavailable)
  const aiResult = await classifyIntent(transcript, language);
  const parsed = { intent: aiResult.intent, gameType: aiResult.gameType, filter: aiResult.filter, target: aiResult.target };
  const lang = language;
  ...
  switch (parsed.intent) {
    ...
    case 'check_progress': { 
      if (patientId) {
        speech = s.progress; 
        data = { action: 'navigate', target: '/patient-report' }; 
      } else {
        speech = s.profileNeeded;
      }
      break; 
  ...
  res.json({ success: true, data: { transcript, parsedIntent: parsed, speech, confidence: aiResult.confidence, intentSource: aiResult.source, ...data } });
});
