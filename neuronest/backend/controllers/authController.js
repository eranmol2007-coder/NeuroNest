...
const verifyOtp = asyncHandler(async (req, res) => {
  ...
  if (purpose === 'caregiver') {
    ...
    if (!caregiver) {
      // If a caregiver tries to sign up independently, create their record
    ...
    if (!user) {
      user = await User.create({
        ...
        role: 'caregiver',
        patientId: caregiver.linkedPatientId || undefined,
        caregiverId: caregiver._id,
        isVerified: true,
      });
    } else {
      // Existing user found — fully overwrite with caregiver identity.
      // IMPORTANT: always replace patientId from the caregiver record,
      // never inherit a stale patientId from a previous patient account
      // on the same phone/email — that would show the wrong patient's data.
      if (caregiverName) user.name = caregiverName;
      user.role = 'caregiver';
      user.caregiverId = caregiver._id;
      user.patientId = caregiver.linkedPatientId || null;
      await user.save();
    }
...
const getMe = asyncHandler(async (req, res) => {
  ...
  if (user.role === 'caregiver') {
    // Caregivers: always load patient from the caregiver record — never from user.patientId
    // which could be stale from a previous patient account on the same phone/email
    if (caregiver?.linkedPatientId) {
      patient = await Patient.findById(caregiver.linkedPatientId);
    }
  } else if (user.patientId) {
