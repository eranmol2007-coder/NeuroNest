...
const unlinkCaregiver = asyncHandler(async (req, res) => {
  const { Caregiver, Patient } = getModels();
  const { id } = req.params;
  const caregiver = await Caregiver.findById(id);
  if (!caregiver) {
  ...
  caregiver.linkedPatientId = null;
  await caregiver.save();
  if (patientId) {
    await Patient.findByIdAndUpdate(patientId, { caregiverId: null });
  }
  res.json({ success: true, data: caregiver });
});
...
module.exports = {
  ...
  linkCaregiverToPatient, unlinkCaregiver, getCaregiverDashboard,
};
