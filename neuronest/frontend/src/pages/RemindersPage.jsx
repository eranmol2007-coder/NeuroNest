...
export default function RemindersPage() {
  ...
  const storageKey = patient?._id ? `neuronest_reminders_${patient._id}` : 'neuronest_reminders';
  const [reminders, setReminders] = useState(() => {
    try {
      // Try patient-scoped key first
      const saved = localStorage.getItem(storageKey);
      if (saved) return JSON.parse(saved);
      // Migrate from old unscoped key if patient-scoped key is empty
      if (patient?._id) {
        const oldSaved = localStorage.getItem('neuronest_reminders');
        if (oldSaved) {
          const parsed = JSON.parse(oldSaved);
          // Migrate: save to new key and clear old key
          localStorage.setItem(storageKey, oldSaved);
          localStorage.removeItem('neuronest_reminders');
          return parsed;
        }
      }
    } catch {}
  ...
  // Re-load reminders when the patient changes (e.g. switching accounts)
  useEffect(() => {
    try {
      const saved = localStorage.getItem(storageKey);
      if (saved) {
        ...
        // Fallback: migrate from old unscoped key
        const oldSaved = localStorage.getItem('neuronest_reminders');
        if (oldSaved) {
          const parsed = JSON.parse(oldSaved);
          localStorage.setItem(storageKey, oldSaved);
  ...
  // Save reminders to localStorage — only after initial load is complete
  useEffect(() => {
    if (!remindersLoaded) return;
    localStorage.setItem(storageKey, JSON.stringify(reminders));
  }, [reminders, storageKey, remindersLoaded]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('neuronest_reminders_updated', handleUpdate);
    return () => window.removeEventListener('neuronest_reminders_updated', handleUpdate);
  }, [storageKey]);
