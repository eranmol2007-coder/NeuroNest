...
function matchQuery(doc, query) {
  for (const [key, val] of Object.entries(query)) {
    if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
      ...
      const docVal = doc[key] === undefined ? undefined : doc[key];
      if (val === true && docVal === undefined) continue;
  ...
  findById(id) {
    ...
    const proxy = new Proxy(doc, {
      get(target, prop) {
        ...
        if (prop === 'populate') {
          return () => proxy; // No-op for in-memory DB, prevents crashes
        }
    ...
    function makeDocProxy(doc) {
      ...
      return new Proxy(doc, {
        get(target, prop) {
          ...
          if (prop === 'toJSON' || prop === 'toObject') return () => ({ ...target });
          return target[prop];
        },
      });
    }
...
module.exports = {
  ...
  PersonalStory: makeModel('personalstories'),
  StoryQuiz: makeModel('storyquizzes'),
};
