...
import { cacheGet, cacheSet, cacheClear } from '../services/offlineSync';
...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(async (token, userData, patientData, caregiverData) => {
    // Clear any stale cache from a previous session before storing the new user
    await cacheClear('activeUser');
    await cacheClear('activePatient');
    await cacheClear('activeCaregiver');
  ...
  const logout = useCallback(async () => {
    ...
    // Clear IndexedDB cache so no stale data bleeds into the next session
    await cacheClear('activeUser');
    await cacheClear('activePatient');
    await cacheClear('activeCaregiver');
    setUser(null);
